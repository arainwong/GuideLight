...
SHOCK_2X_ROUTE_DIR="$ROOT/$SUB5_SCENARIO/shock_peak2xplus_h07_h08"
SHOCK_2P5_ROUTE_DIR="$ROOT/$SUB5_SCENARIO/shock_peak2p5plus_h07_h08"
LOG_DIR="$ROOT/$SUB4_SCENARIO/eval_logs"

if [ ! -x "$PYTHON" ]; then echo "Missing Python executable: $PYTHON" >&2; exit 2; fi
if [ ! -f "$MODEL_PATH" ]; then echo "Missing checkpoint: $MODEL_PATH" >&2; exit 2; fi
for dir in "$BASELINE_ROUTE_DIR" "$SHOCK_2X_ROUTE_DIR" "$SHOCK_2P5_ROUTE_DIR"; do
  if [ ! -d "$dir" ]; then echo "Missing route dir: $dir" >&2; exit 2; fi
done
...
if [ "$NUM_EPS" -eq 1 ]; then ROUTES="route_1"; else ROUTES="routes_1_${NUM_EPS}"; fi
INDICES="1-${NUM_EPS}"
BASELINE_PORT=$PORT_START
SHOCK_2X_PORT=$((PORT_START + 1))
...
completed_count() {
  if [ ! -d "$LOG_DIR/$1" ]; then echo 0; return; fi
  find "$LOG_DIR/$1" -path '*/trial_*/plt_*.pkl' -type f 2>/dev/null | wc -l
}

run_condition() {
  local condition=$1 route_dir=$2 port=$3 start=$4 duration=$5
  local existing log_path
...
mkdir -p "$LOG_DIR"
cd "$ROOT"
echo "[start] model=$MODEL routes=$NUM_EPS ports=$BASELINE_PORT/$SHOCK_2X_PORT/$SHOCK_2P5_PORT"
echo "[routes] baseline=$BASELINE_ROUTE_DIR"
echo "[routes] shock_2x=$SHOCK_2X_ROUTE_DIR"
...
for SEGMENT in h00_h06 h06_h12 h12_h18 h18_h24; do
  ...
  BASELINE_CONDITION="sub5_peakdemandplus_baseline_real_${ROUTES}_${MODEL_TAG}_${SEGMENT}"
  SHOCK_2X_CONDITION="sub5_peakdemandplus_shock_2x_${ROUTES}_${MODEL_TAG}_${SEGMENT}"
  SHOCK_2P5_CONDITION="sub5_peakdemandplus_shock_2p5_${ROUTES}_${MODEL_TAG}_${SEGMENT}"
  BASELINE_CONDITIONS="${BASELINE_CONDITIONS:+${BASELINE_CONDITIONS},}${BASELINE_CONDITION}"
  SHOCK_2X_CONDITIONS="${SHOCK_2X_CONDITIONS:+${SHOCK_2X_CONDITIONS},}${SHOCK_2X_CONDITION}"
  ...
  run_condition "$SHOCK_2X_CONDITION" "$SHOCK_2X_ROUTE_DIR" "$SHOCK_2X_PORT" "$START" "$DURATION"
  run_condition "$SHOCK_2P5_CONDITION" "$SHOCK_2P5_ROUTE_DIR" "$SHOCK_2P5_PORT" "$START" "$DURATION"
done

plot_case() {
  local shock_label=$1 shock_conditions=$2 shock_route_dir=$3
  local output_dir="$ROOT/$SUB5_SCENARIO/analysis_plots/${ROUTES}_h00_h24_5min_${MODEL_TAG}_peakdemandplus_${shock_label}"
  mkdir -p "$output_dir"
  echo "[plot] $shock_label demand-flow sync -> $output_dir"
  ...
  "$PYTHON" "$SUB4_SCENARIO/plot_shock_analysis.py" sync-demand \
    --indices "$INDICES" --shock-conditions "$shock_conditions" \
    --baseline-conditions "$BASELINE_CONDITIONS" \
    --baseline-route-dir "$BASELINE_ROUTE_DIR" --shock-route-dir "$shock_route_dir" \
    --window-begin 0 --window-end 86400 --output-dir "$output_dir"
}

plot_case shock_2x "$SHOCK_2X_CONDITIONS" "$SHOCK_2X_ROUTE_DIR"
plot_case shock_2p5 "$SHOCK_2P5_CONDITIONS" "$SHOCK_2P5_ROUTE_DIR"

echo "[complete] wrote demand-flow synchronization results under $ROOT/$SUB5_SCENARIO/analysis_plots"
