...
class TSCSimulator:
    ...
    def _init_sim(self, sumocfg_file, seed, episode_length_time, gui=False):
        ...
        if sumocfg_file.split("/")[-1] in ['grid4x4.sumocfg', 'arterial4x4.sumocfg', 'base.sumocfg']:
            ...
            sumocfg_root = xml.etree.ElementTree.parse(sumocfg_file).getroot()
            net_name = sumocfg_root.find('input/net-file').attrib['value']
            net = os.path.join(os.path.dirname(sumocfg_file), net_name)
            route_dir = self.config.get("route_dir", "sumo_fenglin_base_sub1")
            if os.path.isabs(route_dir):
                route = os.path.join(route_dir, route_name)
            else:
                route = "/".join(sumocfg_file.split("/")[:-2] + [route_dir, route_name])
            command = [
        ...
        if self.config.get("skip_pre_start_departures", False):
            # Start segmented evaluations with an empty network at the segment boundary.
            command += ['--begin', str(self._current_time)]
        if self.config.get("use_fixed_seed", False):
