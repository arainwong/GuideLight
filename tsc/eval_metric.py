...
from tsc.eval_config import config, map_configs, name, metrics #从evalconfig文件中读取需要评测的地图和实验信息

log_dir = os.path.join(os.getcwd(), config['environment']['output_path_head'])#保存评估文件的地址
trials = [folder for folder in next(os.walk(log_dir))[1]]#重复实验对应的文件夹

for metric in metrics:
    output_file = os.path.join(log_dir, 'avg_{}.py'.format(metric))
    run_avg = dict()

    for trial in trials:#重复实验
        for map_name in map_configs:#地图名
            average_per_episode = []
            for i in range(3, 1000, 3):#偶数结尾的文件才有数据
                #trip_file_name = log_dir + names + os.sep + names + "_" + map_name + ".sumocfg_{}_trip.xml".format(i)
                trip_file_name = os.path.join(log_dir, trial, name + "_" + map_name + ".sumocfg_{}_trip.xml".format(i))
